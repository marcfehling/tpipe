                          ...
                          std::vector<CellData<3>> &cell_data_3d)
{
  ...
  GridGenerator::hyper_ball_balanced(circle, center, radius);

  // extrude to cylinder
  const unsigned int n_slices = n_sections + 1;
  const double       height   = 1.;
  Triangulation<3>   cylinder;
  GridGenerator::extrude_triangulation(
    circle, n_slices, height, cylinder, /*copy_manifold_ids=*/false);

  // extract coarse mesh description (as return parameters)
  const auto coarse_mesh = GridTools::get_coarse_mesh_description(cylinder);
  vertices_3d            = std::get<0>(coarse_mesh);
  cell_data_3d           = std::get<1>(coarse_mesh);

#ifdef DEBUG
  // create triangulation
  Triangulation<3> tria(Triangulation<3>::MeshSmoothing::none,
                        /*check_for_distorted_cells=*/true);

  SubCellData subcell_data = std::get<2>(coarse_mesh);
