...
Tensor<2, 3>
compute_rotation_matrix(Tensor<1, 3> a0, Tensor<1, 3> a1, Tensor<1, 3> b0, Tensor<1, 3> b1)
{
             ...
             bool         is_left                    = true)
{
    ...
    else
    {
      ...
      for(int i = 0; i < 8; i++)
      {
        degree_intersection_normal[i] =
          get_degree(normal_intersection_planes[i], normals_rotation_parent[i]);
      }
  ...
  unsigned int range_global =
    !is_child ?
      0 :
      ((n_intersections + (is_left ? 0 : n_intersections)) + 2) * lung_number_of_vertices_2d;
